...
export { HeroCanvasFallback } from "./hero-canvas-fallback";
...
export function HeroCanvas({ pointerX, pointerY }: HeroCanvasProps) {
  ...
  // Scrolling past the hero should stop the render loop entirely, not just
  // hide it — an off-screen canvas still burns a full GPU frame budget.
  const isVisible = useInViewport(containerRef, { rootMargin: "120px" });
  ...
  return (
    <div ref={containerRef} className="absolute inset-0 z-0 overflow-hidden" aria-hidden="true">
      <Canvas
        ...
        // Antialiasing is redundant on a soft, blurred blob and costs real time
        // on integrated GPUs; capping dpr at 1.5 keeps 4K displays sane.
        dpr={[1, 1.5]}
